...
int main(int argc, const char *argv[])
{
	...
	addr_len = lookup_address(hostname, port, &addr);
    if (addr_len < 0) {
        fprintf(stderr, "%s:%s: %s\n", hostname, port, gai_strerror(addr_len));
        exit(EXIT_FAILURE);
    }
	...
	int bindSock = bind(sock, addr, addr_len);
		if(bindSock < 0)
		{
			perror("Bind Error");
			exit(EXIT_FAILURE);
    ...
    {
        if (handle_request(req, stream) < 0) {
            return 1;
        } else {
			fclose(stream);   
            close(client);
            ...
            code = handle_request(req, stream);
            if (code < 0) 
            {
